...
1. Supabase SQL Editor에서 `migrations/20260916000000_guest_memories.sql` 전체를 한 번 실행합니다.
...
RLS로 하객은 본인 기록만 조회·추가할 수 있습니다. 다른 하객의 기록 조회, 수정, 삭제는 허용하지 않습니다. 사진 버킷은 비공개이며 PNG 한 장은 최대 2MB입니다. 관리자는 Supabase 대시보드에서 전체 기록을 봅니다. 계정 비밀번호를 공유하지 말고 필요한 관리자만 프로젝트에 초대합니다.
...
익명 인증 정보는 현재 브라우저에 저장됩니다. 브라우저 데이터를 지우거나 다른 기기로 접속하면 기존 기록을 본인이 조회할 수 없지만, 서버 기록은 유지되어 관리자는 계속 볼 수 있습니다. 기존 로컬 체험판 메시지를 자동으로 서버에 올리지는 않습니다.
...
이 테스트는 실제 DB 권한 검증을 대체하지 않습니다. 초기 설정 후 별도 익명 하객 두 명으로 본인 기록 조회, 타인 조회 차단, 무인증 쓰기 차단을 실제 Supabase에서 확인합니다.
...
Photo results and message forms reuse existing panels and buttons. While sending, disable repeat submission and show a text status with `aria-busy` or `role=status`. A failed message keeps its draft; a failed photo shows an explicit retry button. Completion text appears only after the server acknowledges storage. Guest records are private to the author and the couple; the invitation explains this before submission.
