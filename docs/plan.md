...
- [x] 시나리오 데이터 스키마 정의 후 `data/scenario.ts`에 Q1~Q3 입력 (scenario.md 표 내용을
      그대로 옮김). Q3(신랑측/신부측)는 오답이 없는 분기 선택이라 두 선택지 모두
      `correct: true`로 표현 — 힌트/재시도 로직이 자연히 발동하지 않음
- [x] `QuizModal` 컴포넌트: 질문 텍스트, 선택지 버튼(2~5개 가변, 옵션 개수에 따라 패널
      높이 자동 계산), 정답 콜백, 오답 시 리액션 텍스트 표시 + "힌트 보기" 버튼 노출
- [x] 오답 시: 모달 유지 + 리액션 문구(옵션별 문구 없으면 공통 폴백 문구), 재시도 무제한
- [x] 정답 시: 모달(컨테이너) 파괴 + `onCorrect` 콜백 실행
- [x] `ArrowGuide` 컴포넌트: 좌표 배열마다 화살표 텍스트 오브젝트 표시, 마지막 지점에
      `TriggerZone`을 재사용해 도달 시 화살표 제거 + 완료 콜백 (아직 실제 씬에는 미연결 —
      Phase 3의 `VenueHallScene`에서 사용 예정)
- [x] `systems/placeholderQuizRoom.ts`: Phase 1의 플레이스홀더 방 패턴을 재사용하는 공통
      헬퍼. `CarRouteScene`(Q1)·`SubwayRouteScene`(Q2)에 실제로 연결해 트리거 존 도달 →
      퀴즈 모달 → 정답 시 페이드 → `VenueLobbyScene`(플레이스홀더)까지 실제 동작하도록 구성

**완료 기준**: 플레이스홀더 맵 위에서 트리거 존에 닿으면 퀴즈 모달이 뜨고, 오답 선택 시
힌트가 뜨며, 정답 선택 시 모달이 닫히고 이벤트가 발생한다. → **검증 완료.** 헤드리스
브라우저로 자차(Q1, 3지선다)·지하철(Q2, 5지선다) 두 경로 모두 오답→리액션→힌트 노출→
정답→페이드→`VenueLobbyScene` 전환까지 실제 탭 시뮬레이션으로 확인함.
...
- [x] `Scale.FIT` 기준으로 다양한 화면비에서 레이아웃 깨짐 없는지 확인 — 헤드리스 브라우저로
      6개 실제 기기 해상도(iPhone SE 320×568, iPhone X 375×812, iPhone 14 390×844,
      iPhone 14 Pro Max 430×932, 일반 Android 360×800, Pixel 412×915)에서 홈 화면과
      가장 콘텐츠가 많은 5지선다 퀴즈 모달까지 확인. `Scale.FIT`이라 어떤 화면비에서도
      **잘리는 부분은 없음**(레터박스만 생김). 다만 가장 작은 화면에서 트리거 존 라벨
      텍스트가 박스를 살짝 벗어나는 게 보여 폰트 크기(18→15px)와 wordWrap 여백을 조정함
- [x] OG 메타 태그(`og:title`, `og:description`, `og:image` 등) 추가 — `index.html`에 반영.
      `og:image`는 아직 실제 아트가 없어 임시로 헤더/디자인 문서의 웨딩 톤(아이보리·블러쉬핑크·
      그리너리·골드)을 반영한 1200×630 카드를 만들어 `public/og-image.png`로 추가
      (Phase 5에서 실제 아트로 교체 예정)
- [x] 로딩 속도 확인 — 프로덕션 빌드(`vite preview`) 기준, 캐시 비우고 "Slow 4G"급
      네트워크(다운로드 1.6Mbps, RTT 150ms)로 에뮬레이션했을 때 캔버스가 뜨기까지 약 2.2초,
      전체 전송량은 gzip 기준 약 363KB (JS 번들이 대부분). 플레이스홀더 단계라 가벼운 편이며,
      Phase 5에서 실제 픽셀 아트 에셋이 추가되면 반드시 재측정 필요
- [ ] 실기기(iOS Safari, Android Chrome) + 카카오톡 인앱 브라우저에서 탭투무브 동작 확인 —
      **이 저장소 환경에는 실제 모바일 기기가 없어 직접 확인 불가.** 배포 후 사용자가 실기기로
      확인 필요 (아래 참고)
- [ ] 정적 호스팅 배포 (Vercel/Netlify) 및 실제 링크로 카톡 공유 테스트 — **이 환경에는 배포
      도구/계정이 연결되어 있지 않아 실행하지 못함.** 사용자가 원하는 호스팅 방식을 정하면
      그에 맞춰 배포 설정(빌드 커맨드, 환경변수 등)을 추가할 수 있음
...
> **중요한 한계**: 이 세션(Claude Code)에는 이미지 생성 도구가 연결되어 있지 않다. 즉
> 실제 일러스트/픽셀아트 파일을 그려낼 수 없다. 아래 두 항목만 "진짜 에셋"으로 완료했고,
> 나머지(캐릭터 일러스트, 배경 타일셋, 오브젝트 아이콘)는 **코드로 생성한 대체 그래픽**으로
> 대신했다 — 실제 아트가 생기면 교체가 필요하다.

- [x] **픽셀 폰트 선정 + 라이선스 확인**: [Galmuri](https://github.com/quiple/galmuri) 11
      (OFL-1.1, 완전 재배포 허용)을 채택. `fonttools`로 이 프로젝트가 실제 쓰는 글자만
      서브셋해 504KB → 8KB(Regular), 166KB → 7KB(Bold)로 축소 후 `game/public/fonts/`에
      자체 호스팅, 라이선스 원문도 함께 커밋(`fonts/LICENSE.txt`). 모든 씬의 텍스트에 적용
      완료. 새 대사/카피가 추가되면 폰트에 없는 글자가 나올 수 있으니, 그때는
      `docs/plan.md`의 서브셋 절차(아래 메모)를 다시 돌려야 한다
- [x] **플레이어/신부 캐릭터**: 실제 스프라이트 대신 `systems/pixelCharacter.ts`가 Phaser
      Graphics로 그리는 12×16 "도트 블록" 캐릭터로 대체 (색상 팔레트만 다르게 해서 하객/신부
      구분). idle + 2프레임 걷기 애니메이션 포함, `TextureFilterMode.NEAREST`로 확대해도
      각지게(픽셀아트처럼) 보이도록 처리. 4방향 표정/걷기는 없음 — 단순 좌우 다리 교차만 구현
- [x] **UI 컴포넌트 아트**: `QuizModal`에 골드 테두리 + 그림자 오프셋(눌린 버튼처럼 보이는
      효과)을 입혀 단순 사각형보다 "게임 UI"에 가깝게 다듬음
- [x] **바닥/타일 표현**: `systems/pixelFloor.ts`로 단색 사각형 대신 체커보드 형태의
      은은한 타일 패턴을 그려 넣음 (실제 타일셋은 아님, map.md 사진 기반 타일셋은 여전히
      미착수)
- [ ] 배경 타일셋: 도로/주차장, 지하철역, 예식장 로비/복도/신부대기실/연회장/축의대/포토부스
      (map.md 참고 사진 기반) — **미착수, 실제 아트 필요**
- [ ] 오브젝트: 유도선(노랑/핑크/파랑), 출구 표지판, 포토테이블, 축의대 데스크 등 세부
      오브젝트 일러스트 — **미착수, 실제 아트 필요** (화살표 가이드 아이콘만 이모지 `⬇`로
      임시 표시 중)

에셋이 준비되는 대로 지금의 코드-생성 플레이스홀더를 하나씩 교체한다. 텍스처 키/규격이
`ensurePixelCharacter`, `ensureCircleTexture` 같은 헬퍼 함수 뒤에 감춰져 있어서, 실제
이미지 파일이 생기면 해당 헬퍼 내부만 `scene.load.image(...)` 방식으로 바꾸면 되고 호출부
(씬 코드)는 거의 안 건드려도 된다.

### 픽셀 사이드퀘스트: 트리거 존이 "지나가기만 해도" 발동하던 버그

Phase 5 작업 중 실제로 재현된 버그: `HomeSelectScene`에서 한 트리거 존(A) 쪽으로 탭했다가
경로 도중 되돌아서 다른 트리거 존(B)으로 다시 탭하면, BFS 최단경로가 A와 B 사이 어딘가에서
B 쪽 존을 그냥 스쳐 지나가는 경로를 고를 수 있고, 이때 overlap이 걸려 B가 아니라 A가 아닌
엉뚱한 선택지가 그냥 지나가다 발동해버렸다. `TriggerZone`이 "겹침"만 보고 판정했기 때문 —
이동 중(속도 있음)인지, 도착해서 멈췄는지 구분하지 않았던 게 원인. `player.body.velocity`가
거의 0일 때만 발동하도록 고쳐서, 실제 목적지가 아닌 경유지에서는 발동하지 않게 했다
(`systems/TriggerZone.ts`).
